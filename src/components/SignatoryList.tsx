      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
                ...
                <div className="flex items-center space-x-2">
                  <h3 className="font-semibold text-sm truncate flex items-center gap-1">
                    ...
                    {signatory.verified && (
                      ...
                      >
                        <CheckCircle className="h-4 w-4 text-blue-500 shrink-0 animate-pulse" />
                      </motion.span>
                    )}
                  </h3>
                </div>
          ...
          {signatory.message && (
            <div className="mb-4">
              <div className="relative">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed pl-4 group-hover:text-foreground transition-colors">
                  {truncateMessage(signatory.message)}
                </p>
                {signatory.message.length > 150 && (
                  <button
                    onClick={() => setSelectedSignatory(signatory)}
                    className="text-xs text-flame-600 hover:text-flame-700 mt-1 flex items-center gap-1 pl-4 transition-colors"
                  >
          ...
          <div className="space-y-2 mb-4">
            {signatory.location && (
              <div className="flex items-center space-x-2 text-xs text-muted-foreground group-hover:text-foreground transition-colors">
            ...
            <div className="flex items-center space-x-3 pt-3 border-t border-flame-200/20">
              {signatory.social.twitter && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-muted-foreground hover:text-flame-600 transition-colors hover:scale-125 transform"
                  title={`Follow ${signatory.name || 'Anonymous'} on Twitter`}
                  aria-label={`Follow ${signatory.name || 'Anonymous'} on Twitter`}
                >
              ...
              {signatory.social.linkedin && (
                <a
                  ...
                  rel="noopener noreferrer"
                  className="text-muted-foreground hover:text-flame-600 transition-colors hover:scale-125 transform"
                  title={`Connect with ${signatory.name || 'Anonymous'} on LinkedIn`}
                  aria-label={`Connect with ${signatory.name || 'Anonymous'} on LinkedIn`}
                >
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
              ...
              {signatory.social.github && (
                <a
                  ...
                  rel="noopener noreferrer"
                  className="text-muted-foreground hover:text-flame-600 transition-colors hover:scale-125 transform"
                  title={`View ${signatory.name || 'Anonymous'}'s GitHub profile`}
                  aria-label={`View ${signatory.name || 'Anonymous'}'s GitHub profile`}
                >
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
    ...
    <AnimatePresence>
      {selectedSignatory && (
        <motion.div
          ...
          onClick={() => setSelectedSignatory(null)}
        >
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.9, opacity: 0 }}
            className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto relative"
          ...
          >
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="text-xl font-semibold flex items-center gap-2">
                      {selectedSignatory.name || 'Anonymous'}
                  ...
                  {selectedSignatory.organization && (
                    <p className="text-muted-foreground">{selectedSignatory.organization}</p>
                  )}
                  {selectedSignatory.title && (
                    <p className="text-sm text-muted-foreground">{selectedSignatory.title}</p>
                  )}
                </div>
              </div>
              <button
            ...
            <div className="mb-6">
              <div className="relative">
                <Quote className="absolute -top-2 -left-2 h-6 w-6 text-flame-300 opacity-30" />
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 pl-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              {selectedSignatory.location && (
                <div className="flex items-center space-x-2 text-muted-foreground">
                  ...
                  <span>{selectedSignatory.location}</span>
                </div>
              )}
              ...
              {selectedSignatory.website && (
                <div className="flex items-center space-x-2 text-muted-foreground">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:text-flame-600 transition-colors truncate"
                  >
                    {selectedSignatory.website.replace(/^https?:\/\//, '')}
                  </a>
                </div>
              )}
              ...
              <div className="flex items-center space-x-2 text-muted-foreground">
                ...
                <span>{new Date(selectedSignatory.created_at).toLocaleDateString()}</span>
              </div>
            </div>

            {/* Social Links */}
            {(selectedSignatory.social?.twitter || selectedSignatory.social?.linkedin || selectedSignatory.social?.github) && (
              <div className="flex items-center space-x-4 pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
                <span className="text-sm text-muted-foreground">Connect:</span>
                {selectedSignatory.social.twitter && (
                  <a
                    href={`https://twitter.com/${selectedSignatory.social.twitter}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                {selectedSignatory.social.linkedin && (
                  <a
                    href={`https://linkedin.com/in/${selectedSignatory.social.linkedin}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-muted-foreground hover:text-flame-600 transition-colors"
                  >
                ...
                {selectedSignatory.social.github && (
                  <a
                    href={`https://github.com/${selectedSignatory.social.github}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-muted-foreground hover:text-flame-600 transition-colors"
                  >
